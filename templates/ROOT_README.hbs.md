...
⚠️ Over the past few years, this repo has become a dumping ground for a variety of packages unrelated to the core problems Quilt, and it's stewards the Admin Web Foundation team aims to solve. Before submitting a pull request, please speak with the Admin Web Foundations team on guidance as to whether a package might belong in Quilt. The Admin Web Foundations team's focus is on the `web` codebase. If you're proposing a package has not already been widely used in the the `web` codebase it is unlikely that it would be merged into Quilt.
