...
describe('getTimeZoneOffset()', () => {
  it('returns the time zone offset between Australia/Perth and UTC', () => {
    const date = new Date('2018-04-20T00:00:00+00:00');
    const timeZoneOffset = getTimeZoneOffset(date, 'Australia/Perth', 'UTC');

    expect(timeZoneOffset).toStrictEqual(480);
  });

  it('returns the time zone offset between America/Toronto and UTC when DST is in effect', () => {
    const date = new Date('2018-04-20T00:00:00+00:00');
    const timeZoneOffset = getTimeZoneOffset(date, 'America/Toronto', 'UTC');

    expect(timeZoneOffset).toStrictEqual(-240);
  });

  it('returns the time zone offset between America/Toronto and UTC when DST is not in effect', () => {
    const date = new Date('2018-01-20T00:00:00+00:00');
    const timeZoneOffset = getTimeZoneOffset(date, 'America/Toronto', 'UTC');

    expect(timeZoneOffset).toStrictEqual(-300);
  });

  it('returns the time zone offset between America/Toronto and Australia/Perth', () => {
    const date = new Date('2018-04-20T00:00:00+00:00');
    ...
    expect(timeZoneOffset).toStrictEqual(-720);
  });

  it('returns 0 if no arguments passed', () => {
    const timeZoneOffset = getTimeZoneOffset();

    expect(timeZoneOffset).toStrictEqual(0);
  });
  ...
  describe('DST boundaries', () => {
    it('returns regular offset at the start of spring forward DST boundary day', () => {
      ...
      expect(timeZoneOffset).toStrictEqual(600);
    });
    ...
    it('returns DST offset at the end of spring forward DST boundary day', () => {
      ...
      expect(timeZoneOffset).toStrictEqual(660);
    });
