...
describe('createMessengerFunctionStrategy()', () => {
  ...
  it('resolves calls of multiple functions even if the results resolve out of order', async () => {
    ...
    const resultFuncOne = destination.fromWire(
      source.toWire(() => valueOne.promise)[0],
    );
    const resultFuncTwo = destination.fromWire(
      source.toWire(() => valueTwo.promise)[0],
    );

    const resultOne = resultFuncOne();
    const resultTwo = resultFuncTwo();
