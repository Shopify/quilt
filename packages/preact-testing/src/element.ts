...
const portalType = createPortal(
  React.createElement('div', null),
  document.createElement('div'),
).type;
function isPortal(vnode: any) {
  return vnode.type === portalType;
}
...
export class Element<Props> implements Node<Props> {
  ...
  get isDOM() {
    return typeof this.tree.type !== 'function';
  }
  ...
  text(): string {
    const {instance, allChildren, tree} = this;
