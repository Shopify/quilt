...
import {createRoot, fiberToElement, getInternals} from './compat';
import {
  Node,
  Predicate,
  FunctionKeys,
...
type Render = (element: React.VNode<unknown>) => React.VNode<unknown>;
...
export class Root<Props> implements Node<Props> {
  ...
  private reactRoot: {render(vnode: any): void; unmount(): void} | null = null;
  ...
  constructor(
    private tree: React.VNode<Props>,
  ...
  private update() {
    if (this.wrapper == null) {
      ...
      const topElement = fiberToElement(rootFiber, this);
...
function defaultRender(element: React.VNode<unknown>) {
  return element;
}
